...
The microservice is known to be built and run successfully under **Ubuntu Server (Ubuntu 22.04.3 LTS x86-64)**. Install the necessary dependencies (`build-essential`, `tcc`, `libglib2.0-dev`, `docker.io`):

```
$ sudo apt-get update && \
  sudo apt-get install build-essential tcc libglib2.0-dev docker.io -y
```
